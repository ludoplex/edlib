...
if not result or result["editDistance"] != 3:
    testFailed = True
...
if (
    len(test_getNiceNW['query_aligned']) != 18
    or len(test_getNiceNW['target_aligned']) != 18
):
    testFailed = True
if test_getNiceNW['query_aligned'] != 'TAAGGATGGTCCCAT-TC':
    testFailed = True
if test_getNiceNW['matched_aligned'] != '-||||--||||.|||-||':
    testFailed = True
if test_getNiceNW['target_aligned'] != '-AAGG--GGTCTCATATC':
    testFailed = True
...
if (
    len(test_getNiceHW['query_aligned']) != 18
    or len(test_getNiceHW['target_aligned']) != 18
):
    testFailed = True
if test_getNiceHW['query_aligned'] != 'TAAGGATGGTCCCAT-TC':
    testFailed = True
if test_getNiceHW['matched_aligned'] != '-||||--||||.|||-||':
    testFailed = True
if test_getNiceHW['target_aligned'] != '-AAGG--GGTCTCATATC':
    testFailed = True
...
if (
    len(test_getNiceSHW['query_aligned']) != 18
    or len(test_getNiceSHW['target_aligned']) != 18
):
    testFailed = True
if test_getNiceSHW['query_aligned'] != 'TAAGGATGGTCCCAT-TC':
    testFailed = True
if test_getNiceSHW['matched_aligned'] != '-||||--||||.|||-||':
    testFailed = True
if test_getNiceSHW['target_aligned'] != '-AAGG--GGTCTCATATC':
    testFailed = True

result_taskDistance = edlib.align(query="TAAGGATGGTCCCATTC", target="AAGGGGTCTCATATC", mode="NW", task="distance")
if result_taskDistance["cigar"] is not None:
    testFailed = True

# Empty characters.
result = edlib.align("", "elephant")
testFailed = testFailed or not result or result["editDistance"] != 8
result = edlib.align("telephone", "")
testFailed = testFailed or not result or result["editDistance"] != 9
result = edlib.align("", "elephant", mode="HW")
testFailed = testFailed or not result or result["editDistance"] != 0
...
testFailed = (
    testFailed
    or not result
    or result["editDistance"] != 5
    or result["alphabetLength"] != 12
)
...
testFailed = testFailed or not result or result["editDistance"] != 256
